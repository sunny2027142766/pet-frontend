...
const generateUniqueMenus = (menus) => {
  const menuIds = {};
  ...
  return (
    <Stack spacing={3}>
      <TextField
        ...
        required
        error={emailError}
        helperText={emailError ? emailErrorHelper : ""}
        InputProps={{
          endAdornment: (
    ...
    if (!hasError) {
      ...
      if (res.code === 200 && res.msg === "登录成功") {
        ...
        if (userInfoRes.code === 200) {
          ...
          const frontMenu = generateUniqueMenus(menus);
          if (frontMenu.length === 0) {
            setTip("您没有访问前台的权限");
            setTipOpen(true);
            setTimeout(() => router.push("/403"), 1000);
            return;
          }
          if (frontMenu.length > 0) {
            ...
            setTimeout(() => router.push(firstMenu.path), 1000);
          }
        }
      } else {
        setTip(res.msg);
      ...
      <Stack alignItems="center" justifyContent="center" sx={{ height: 1 }}>
        <Card sx={{ p: 5, width: 1, maxWidth: 420 }}>
          ...
          <LoginForm
            ...
            emailErrorHelper={emailErrorHelper}
            setEmailErrorHelper={setEmailErrorHelper}
            password={password}
